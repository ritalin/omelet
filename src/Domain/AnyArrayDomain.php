...
class AnyArrayDomain extends DomainBase
{
    /**
     * @var DomainBase
     */
    ...
    protected function convertResultsInternal($results, AbstractPlatform $platform, CaseSensor $sensor)
    {
        if (! is_int(key($results))) {
            // case outut parameter returning
            return $this->convertResultBuiltin($results, $platform, $sensor);
        }
        
        if (($this->child instanceof BuiltinDomain)) {
            $selector = 
                function ($v) use ($platform, $sensor) {
                    return $this->convertResultBuiltin($v, $platform, $sensor);
                };
        }
        else {
            $selector = 
        ...
        return array_map($selector, $results);
    }
    ...
    {
        return new AnyArrayDomain($values['child']);
    }
}
